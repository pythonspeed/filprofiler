...
_fil-python: filprofiler/*.c target/release/libpymemprofile_api.a
	gcc -std=c11 $(shell python3.8-config --cflags) -DFIL_SKIP_ALIGNED_ALLOC=1 -DCONDA_EXECUTABLE_FOR_BENCHMARKS=1 -flto -export-dynamic -o ${CONDA_PREFIX}/bin/_fil-python $^ -lpython3.8 $(shell python3.8-config --ldflags)
